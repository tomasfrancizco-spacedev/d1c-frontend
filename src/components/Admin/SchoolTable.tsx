...
export default function SchoolTable() {
  const schoolList = useMemo(
    () => (schools as unknown as School[]).slice().sort((a, b) => a.schoolName.localeCompare(b.schoolName)),
    []
  );
  ...
  const filteredSchoolList = useMemo(() => {
    ...
    return schoolList.filter((school) => {
      if (lowercased.schoolName && !school.schoolName.toLowerCase().includes(lowercased.schoolName)) return false;
      ...
      <table className="min-w-full table-fixed text-left text-sm text-white/90">
        ...
        <tbody>
          {filteredSchoolList.length > 0 ? filteredSchoolList.map((school) => (
            <tr key={`${school.schoolName}-${school.city}`} className="odd:bg-white/0 even:bg-white/[0.03]">
              <td className="min-w-[300px] max-w-[300px] px-4 py-3 whitespace-nowrap truncate">{school.schoolName}</td>
